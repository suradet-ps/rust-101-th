...
fn vec_min(vec: Vec<i32>) -> NumberOrNothing {
    ...
    // Since we start out with nothing computed, this will again be a "number or nothing":
    let mut min = NumberOrNothing::Nothing;
    // We do not have to write a type next to `min`, Rust can figure that out automatically
    // (a bit like `auto` in C++11). Also notice the `mut`: In Rust, variables are immutable
    // per default, and you need to tell Rust if you want to change a variable later.
    ...
    for el in vec {
        ...
        match min {
            ...
            // store it. We will write the function `min_i32` just after we completed this one.
    ...
    // Notice that Rust makes sure you did not forget to handle any case in your `match`.
    // We say that the pattern matching has to be exhaustive.
...
`vec!` is a macro (as indicated by `!`) that constructs a constant `Vec<>` with the given elements.
...
Of course, we would also like to actually see the result of the computation, so we need to print the result. Of course Rust can print numbers, but after calling `vec_min`, we have a `NumberOrNothing`. So let's write a small helper function that prints such things.
