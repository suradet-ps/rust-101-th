...
fn print_digits_v2(b: &BigInt) {
    ...
    // loop if the pattern doesn't match. There's also `if let`, which works similar, but of course
    // without the loopy part.
...
> 
> **Exercise 09.2**: Write a function `iter_ldf` that iterates over the digits with the least-significant digits coming first. Write a testcase for it.
